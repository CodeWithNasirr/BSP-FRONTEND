...
// src/admin/components/MarkAsPaidModal.jsx — Premium responsive (WhatsApp green)
// All business logic preserved from original. Styling unified with premium dark theme.
import React, { useState, useEffect } from "react";
import { adminApi } from "../utils/api";
import { CheckCircle2, X, AlertCircle, FileText, Users, TrendingUp, Wallet, User } from "lucide-react";

const PLAN_PRICING = { "BASIC": 499, "GROWTH": 799, "BUSINESS PRO": 1999 };
const DEFAULT_COMMISSION_RATES = { "BASIC": 20, "GROWTH": 25, "BUSINESS PRO": 30 };

export default function MarkAsPaidModal({ client, open, onClose, onSuccess, subadminCommissionRates = DEFAULT_COMMISSION_RATES }) {
  const [planPrice, setPlanPrice] = useState(0);
  const [commissionPercent, setCommissionPercent] = useState(20);
  const [note, setNote] = useState("");
  const [loading, setLoading] = useState(false);
  ...
  useEffect(() => {
    if (client) {
      setPlanPrice(PLAN_PRICING[client.subscription_plan] || 499);
      setCommissionPercent(subadminCommissionRates[client.subscription_plan] || 20);
      setNote(""); setError(""); setSuccessMessage("");
    }
  }, [client, subadminCommissionRates]);
  ...
  const getSubadminInfo = () => {
    if (client.assigned_subadmin) return { id: client.assigned_subadmin, name: client.assigned_subadmin_name };
    if (client.activated_by) return { id: client.activated_by, name: client.activated_by_username };
    return null;
  };
  const subadmin = getSubadminInfo();
  const commissionAmount = isFreePayment ? 0 : (planPrice * commissionPercent) / 100;
  const platformRevenue = isFreePayment ? 0 : planPrice - commissionAmount;
  const fmt = (v) => { const n = parseFloat(v); return isNaN(n) ? "₹0" : `₹${n.toFixed(0)}`; };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center sm:p-4">
      ...
      <div className="relative bg-[#0d1120] w-full sm:max-w-md sm:mx-4 shadow-2xl overflow-hidden max-h-[95vh] flex flex-col rounded-t-3xl sm:rounded-2xl border border-emerald-500/15">

        {/* Header */}
        <div className="bg-gradient-to-r from-[#075E54] to-[#128C7E] px-4 py-4 sm:px-5 sm:py-5 shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-white/15 backdrop-blur flex items-center justify-center"><CheckCircle2 size={18} className="text-white" /></div>
              <div><h3 className="text-base font-bold text-white">Confirm Payment</h3><p className="text-xs text-[#DCF8C6]">{client.business_name || client.username}</p></div>
            </div>
            <button onClick={onClose} className="text-white/60 hover:text-white transition-colors p-1"><X size={20} /></button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
          ...
          {/* Client card */}
          <div className="bg-white/[0.02] rounded-2xl p-3.5 border border-white/[0.04]">
            <div className="flex items-center gap-3 mb-2.5">
              <div className="w-9 h-9 rounded-full bg-emerald-500/20 flex items-center justify-center shrink-0"><User size={15} className="text-emerald-400" /></div>
              <div className="min-w-0 flex-1">
                <p className="font-bold text-white text-sm truncate">{client.business_name || client.username}</p>
                <p className="text-[10px] text-slate-500">{client.username}</p>
              </div>
            </div>
          ...
          {/* Commission slider */}
          <div className="space-y-3">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1"><Users size={11} />Commission Rate</label>
            <div className="bg-white/[0.02] rounded-xl p-3.5 border border-white/[0.04] space-y-3">
              <div className="flex justify-between text-xs"><span className="text-slate-500">Rate</span><span className="font-bold text-emerald-400">{commissionPercent}%</span></div>
              <input disabled={isFreePayment} type="range" min="0" max="50" step="5" value={commissionPercent} onChange={e => setCommissionPercent(parseInt(e.target.value))}
                className="w-full h-1.5 bg-white/[0.06] rounded-lg appearance-none cursor-pointer accent-emerald-500" />
              <div className="flex gap-1.5">
                {[10, 12.52, 20, 30, 40].map(r => (
                  <button key={r} disabled={isFreePayment} type="button" onClick={() => setCommissionPercent(r)}
                    className={`flex-1 py-1.5 rounded-lg text-[10px] font-bold transition-all ${commissionPercent === r ? "bg-emerald-500 text-white" : "bg-white/[0.03] text-slate-500 border border-white/[0.06]"}`}>{r}%</button>
                ))}
              </div>
            </div>

            <div className="flex items-center gap-2 p-3 bg-amber-500/5 border border-amber-500/10 rounded-xl">
              <input type="checkbox" checked={isFreePayment} onChange={e => setIsFreePayment(e.target.checked)} className="accent-amber-400" />
              <span className="text-[11px] font-semibold text-amber-400">Mark as FREE (no commission)</span>
            </div>

            {/* Revenue split */}
            <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-3.5 space-y-2">
              <div className="flex items-center gap-1.5 mb-2"><TrendingUp size={12} className="text-emerald-400" /><span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider">Revenue Split</span></div>
              <div className="flex justify-between text-xs"><span className="text-slate-400">Client Pays</span><span className="font-bold text-white">{fmt(planPrice)}</span></div>
              <div className="h-px bg-white/[0.04]" />
              <div className="flex justify-between text-xs"><span className="text-slate-400 flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-blue-400" />Platform</span><span className="font-bold text-blue-400">{fmt(platformRevenue)}</span></div>
              <div className="flex justify-between text-xs"><span className="text-slate-400 flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />Commission ({commissionPercent}%)</span><span className="font-bold text-emerald-400">{fmt(commissionAmount)}</span></div>
            </div>
          </div>

          {subadmin && (
            <div className="flex items-center gap-3 p-3 bg-white/[0.02] rounded-xl border border-white/[0.04]">
              <div className="w-7 h-7 rounded-full bg-emerald-500/20 flex items-center justify-center"><Users size={12} className="text-emerald-400" /></div>
              <div className="flex-1 min-w-0"><p className="text-[10px] text-slate-500">{client.activated_by ? "Activated By" : "SubAdmin"}</p><p className="text-xs font-semibold text-white truncate">{subadmin.name || "Unknown"}</p></div>
              <div className="text-right"><p className="text-[10px] text-slate-500">Earns</p><p className="text-xs font-bold text-emerald-400">{fmt(commissionAmount)}</p></div>
            </div>
          )}

          <div>
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1 mb-1.5"><FileText size={11} />Note (Optional)</label>
            <textarea value={note} onChange={e => setNote(e.target.value)} rows={2}
              className="w-full px-3 py-2.5 bg-white/[0.03] border border-white/[0.06] rounded-xl text-xs text-white focus:border-emerald-500/30 focus:outline-none resize-none placeholder:text-slate-600"
              placeholder="e.g., Annual discount applied..." />
          </div>

          <button type="button" onClick={handleSubmit} disabled={loading || planPrice <= 0}
            className="w-full py-3.5 rounded-xl text-sm font-bold text-white bg-[#25D366] hover:bg-[#128C7E] active:bg-[#075E54] transition-all disabled:opacity-50 shadow-lg shadow-emerald-500/20 flex items-center justify-center gap-2">
            {loading ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Processing...</> : <><Wallet size={16} />Confirm Payment</>}
          </button>
        </div>
      </div>
    </div>
