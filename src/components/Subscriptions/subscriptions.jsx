...
const PricingPlans = () => {
    ...
    const applyCoupon = async (plan) => {
        ...
        try {
            ...
            if (discount_applied) {
                setDiscountedPrices((prev) => ({ ...prev, [plan.name]: amount / 100 }));
            ...
            const options = {
                ...
                handler: async function (response) {
                    try {
                        ...
                        const newRemaining = remaining_messages
                        setUsage((prev) => ({ ...prev, remaining: newRemaining }));
        ...
        <section className="bg-gray-50 py-5">
            ...
            <div className="max-w-6xl mx-auto grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 px-6">
                {plans.map((plan) => {
                    ...
                    const displayPrice = discountedPrices[plan.name] || plan.price;
                    const remaining = subscriptionStatus?.get_remaining_messages || (isActivePlan ? plan.included_messages - (usage?.used || 0) : 0); // Use plan's included_messages

                    return (
                        <div
                            key={plan.name}
                            className={`relative rounded-2xl p-6 text-white flex flex-col justify-between min-h-[400px] bg-gradient-to-br ${plan.gradient} transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl ${plan.disabled || isActivePlan ? 'opacity-100 cursor-not-allowed' : ''
                                }`}
                        >
                            <div>
                            ...
                            <div className="space-y-3">
                                ...
                                <button
                                    className={`w-full py-4 rounded-xl text-sm font-semibold transition-all ${selectedPlan === plan.name && !isActivePlan
                                ...
                                >
                                    ...
                                    {isActivePlan && userInfo.api_provider === 'gupshup' && (
                                        <div>
                                            <>
                                                <p>Used: {usage.used} messages</p>
                                                <p>Remaining: {remaining > 0 ? remaining : 0}</p>
                                                {usage.recharge_needed && (
                                                    <button
                                    ...
                                    {isActivePlan && userInfo.api_provider === 'meta' && (
                                        <p>Wallet Balance: ₹{subscriptionStatus?.wallet_balance || 0}</p>
                                    )}
                                    ...
                                    <div>
                                        {remaining >= 0 && (
                                            <button
                                                ...
                                                className="w-full py-2 bg-green-800 text-white rounded-xl text-sm font-semibold hover:bg-yellow-600"
                                            >
                                                Recharge for 1,000+ Messages (₹86)
                                            </button>
                                        )}
                                    </div>
                                )}
