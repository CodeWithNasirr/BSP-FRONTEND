...
import React, { memo, useState } from 'react';
import { Handle, Position } from 'reactflow';
import { MessageSquare, Image, Pencil, Upload, X } from 'lucide-react';
import { uploadFlowMedia } from '../uploadFlowMedia';
import { toast } from 'react-toastify';
import VariablePicker from '../VariablePicker';
import FollowUpEditor from '../FollowUpEditor';
...
const MessageNode = ({ data, selected }) => {
  ...
  const handleFileUpload = async (file) => {
    try {
      ...
      data.message = message;
      setEditing(false);
      toast.success("Media uploaded & saved", { autoClose: 2000 });
    } catch (err) {
  ...
  return (
    <div className={`px-4 py-3 rounded-xl bg-blue-50 dark:bg-blue-500/10 border ${selected ? 'border-blue-400 dark:border-blue-400' : 'border-blue-200 dark:border-blue-500/20'} min-w-[200px] max-w-[300px] transition-colors`}>
      <Handle type="target" position={Position.Top} className="!bg-blue-500" />

      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center">
          <MessageSquare className="mr-2 text-blue-500" size={16} />
          <div className="text-sm font-bold text-blue-800 dark:text-blue-300">Send Message</div>
        </div>
      ...
      {data.stop_flow && (
        <div className="text-xs mt-2 p-1.5 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 rounded-lg border border-red-200 dark:border-red-500/20">
          🚫 Flow stops here
        </div>
      )}
      ...
      {editing ? (
        <div className="space-y-2 mb-2">
          ...
          <div className="flex justify-center">
            <label className="flex items-center gap-2 text-xs cursor-pointer bg-blue-50 dark:bg-blue-500/10 hover:bg-blue-100 dark:hover:bg-blue-500/20 border border-blue-200 dark:border-blue-500/20 text-blue-700 dark:text-blue-300 px-3 py-2 rounded-lg transition-all active:scale-95">
              <Upload size={16} />
              Upload Media
              <input type="file" accept="image/*,video/*,audio/*" hidden onChange={(e) => handleFileUpload(e.target.files[0])} />
            </label>
          </div>

          <label className="flex items-center">
            <input type="checkbox" checked={stopFlow} onChange={(e) => setStopFlow(e.target.checked)} className="mr-2 accent-red-500" />
            <span className="text-xs text-red-600 dark:text-red-400 font-medium">Stop flow after this message</span>
          </label>

          <label className="flex items-center">
            <input type="checkbox" checked={collectAddress} onChange={(e) => setCollectAddress(e.target.checked)} className="mr-2 accent-blue-500" />
            <span className="text-xs text-gray-700 dark:text-gray-300">Collect Address</span>
          </label>

          <label className="flex items-center">
            <input type="checkbox" checked={collectLocation} onChange={(e) => setCollectLocation(e.target.checked)} className="mr-2 accent-blue-500" />
            <span className="text-xs text-gray-700 dark:text-gray-300">Collect Location</span>
          </label>

          {collectLocation && (
            <div className="space-y-1">
              <input type="number" className={inputBaseClass} placeholder="Center Latitude" value={centerLat} onChange={(e) => setCenterLat(e.target.value)} />
              <input type="number" className={inputBaseClass} placeholder="Center Longitude" value={centerLon} onChange={(e) => setCenterLon(e.target.value)} />
              <input type="number" className={inputBaseClass} placeholder="Radius (km)" value={radius} onChange={(e) => setRadius(e.target.value)} min="0" step="0.1" />
            </div>
          )}

          <label className="flex items-center">
            <input type="checkbox" checked={collectInput} onChange={(e) => setCollectInput(e.target.checked)} className="mr-2 accent-blue-500" />
            <span className="text-xs text-gray-700 dark:text-gray-300">Collect Input</span>
          </label>

          {collectInput && (
            <input type="text" className={inputBaseClass} placeholder="Input Key (e.g., device_issue)" value={inputKey} onChange={(e) => setInputKey(e.target.value)} />
          )}
        ...
        <>
          <div className="text-xs bg-white dark:bg-[#111827] p-2 rounded-lg border border-blue-100 dark:border-blue-500/20 text-gray-700 dark:text-gray-300 max-h-[80px] overflow-y-auto">
            {message || 'Empty message'}
          </div>
          {mediaUrl && (
            <div className="flex items-center mt-2 p-1.5 bg-blue-50 dark:bg-blue-500/10 rounded-lg text-xs border border-blue-100 dark:border-blue-500/20">
              <Image size={12} className="mr-1.5 text-blue-500 shrink-0" />
              <span className="truncate w-full text-gray-700 dark:text-gray-300">{mediaUrl}</span>
              <button onClick={handleRemoveMedia} className="text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 ml-2 transition-colors" title="Remove media">
                <X size={12} />
              </button>
            </div>
          )}
          {data.collect_address && (
            <div className="text-xs mt-2 p-1.5 bg-blue-50 dark:bg-blue-500/10 rounded-lg border border-blue-100 dark:border-blue-500/20 text-blue-700 dark:text-blue-300">
              Collects: Address
            </div>
          )}
          {data.collect_location && (
