...
import React, { memo, useState } from 'react';
import { Handle, Position } from 'reactflow';
import { MessageSquare, Plus, Pencil, Trash2, Image, Upload, X } from 'lucide-react';
import { uploadFlowMedia } from '../uploadFlowMedia';
import { toast } from 'react-toastify';
import VariablePicker from '../VariablePicker';
import FollowUpEditor from '../FollowUpEditor';
...
const TextButtonsNode = ({ data, selected }) => {
  ...
  const handleAddButton = () => {
    ...
    const newButton = { text: `Button ${buttons.length + 1}`.slice(0, MAX_BUTTON_TEXT_LENGTH) };
    const updatedButtons = [...buttons, newButton];
    setButtons(updatedButtons);
    data.buttons = updatedButtons;
  };
    ...
    <div className={`px-4 py-3 rounded-xl bg-blue-50 dark:bg-blue-500/10 border ${selected ? 'border-blue-400 dark:border-blue-400' : 'border-blue-200 dark:border-blue-500/20'} min-w-[220px] max-w-[300px] transition-colors`}>
      ...
      <div className="mb-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <MessageSquare className="mr-2 text-blue-500" size={16} />
            <div className="text-sm font-bold text-blue-800 dark:text-blue-300">Media + Text + Buttons</div>
          </div>
      ...
      {editing ? (
        <div className="space-y-2 mb-2">
          ...
          <input className={inputBaseClass} placeholder="Enter footer message" value={footer} onChange={(e) => setFooter(e.target.value)} />
          ...
          {buttons.map((btn, index) => (
            <div key={index} className="flex items-center text-xs gap-1">
              <input
                ...
                className="w-full p-1 border border-blue-200 dark:border-blue-500/30 rounded-lg bg-white dark:bg-[#111827] text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all"
                placeholder="Button text"
              />
              <div className="text-[10px] text-gray-400 dark:text-gray-500 text-right shrink-0">
                {btn.text.length}/{MAX_BUTTON_TEXT_LENGTH}
              </div>
          ...
          <button onClick={handleAddButton} className="flex items-center text-xs text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition-colors">
        ...
        <>
          ...
          <div className="space-y-1">
            {buttons.length > 0 ? (
              ...
              <button onClick={handleAddButton} disabled={buttons.length >= MAX_BUTTONS} className={`flex items-center text-xs ${buttons.length >= MAX_BUTTONS ? 'text-gray-400 dark:text-gray-600 cursor-not-allowed' : 'text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300'}`}>
                <Plus size={12} className="mr-1" /> Add Button
              </button>
            )}
