...
const inputBaseClass = `
  w-full text-xs px-2 py-1 border border-purple-200 dark:border-purple-500/30 rounded-lg
...
const KeywordListenerNode = ({ data, selected }) => {
  ...
  const handleAddKeyword = (groupId) => {
    ...
    if (RESERVED.has(draft)) {
      alert(`'${draft}' is a reserved keyword (used by start/stop/opt-out logic). Pick a different word.`);
      return;
    }
    ...
    const otherGroup = groups.find((g) => g.id !== groupId && g.keywords.includes(draft));
    if (otherGroup) {
      const proceed = window.confirm(
        `'${draft}' is already used in group "${otherGroup.label}". Add it anyway? (First-declared group wins ties.)`
      );
      if (!proceed) return;
    }

    persist(groups.map((g) => (g.id === groupId ? { ...g, keywords: [...g.keywords, draft] } : g)));
    setKwDrafts({ ...kwDrafts, [groupId]: '' });
  };

  const handleRemoveKeyword = (groupId, kw) => {
    persist(groups.map((g) => (g.id === groupId ? { ...g, keywords: g.keywords.filter((k) => k !== kw) } : g)));
  };
  ...
  const totalHandles = groups.length + 1;
  const handlePosition = (index) => `${((index + 1) / (totalHandles + 1)) * 100}%`;

  return (
    <div
      className={`px-4 py-3 rounded-xl bg-purple-50 dark:bg-purple-500/10 border ${
        selected ? 'border-purple-400 dark:border-purple-400' : 'border-purple-200 dark:border-purple-500/20'
      } min-w-[240px] max-w-[320px] transition-colors`}
    >
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center">
          <Filter className="mr-2 text-purple-500" size={16} />
          <div className="text-sm font-bold text-purple-800 dark:text-purple-300">Keyword Router</div>
        </div>
        <button
          onClick={() => setEditing(!editing)}
      ...
      {editing ? (
        <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
          <div className="text-xs text-gray-600 dark:text-gray-400 bg-purple-50 dark:bg-purple-500/10 p-2 rounded-lg border border-purple-100 dark:border-purple-500/20">
            User's next typed message will be matched. Exact match wins over contains. Longer keyword wins. First group wins ties.
          </div>

          {groups.map((group) => (
            <div
              key={group.id}
              className="border border-purple-200 dark:border-purple-500/20 rounded-lg p-2 bg-white dark:bg-[#111827] space-y-2"
            >
              <div className="flex items-center gap-1">
                <input
                  type="text"
                ...
                <button
                  ...
                  title={groups.length <= 1 ? 'At least one group required' : 'Delete group'}
                >
                  <Trash2 size={14} />
                </button>
              </div>
              ...
              {group.keywords.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {group.keywords.map((kw) => (
                    <span
                      key={kw}
                      className="bg-purple-100 dark:bg-purple-500/20 text-purple-800 dark:text-purple-300 text-xs px-2 py-0.5 rounded flex items-center gap-1"
                    >
                      {kw}
                      <button onClick={() => handleRemoveKeyword(group.id, kw)} className="hover:text-purple-900 dark:hover:text-purple-200 transition-colors">
                        <X size={10} />
                      </button>
                    </span>
                  ))}
              ...
              <div className="flex gap-1">
                <input
                  type="text"
                  value={kwDrafts[group.id] || ''}
                  onChange={(e) => setKwDrafts({ ...kwDrafts, [group.id]: e.target.value })}
                  onKeyDown={(e) => handleKeyDown(e, group.id)}
                  placeholder="Add keyword + Enter"
                  className={inputBaseClass}
                />
                <button
                  onClick={() => handleAddKeyword(group.id)}
                  className="bg-purple-500 text-white text-xs px-2 rounded-lg hover:bg-purple-600 transition-all active:scale-95 shadow-sm flex items-center"
                >
                  <Plus size={12} />
                </button>
              </div>
              ...
              {group.keywords.length === 0 && (
                <div className="text-xs text-amber-600 dark:text-amber-400">⚠️ No keywords. This group will never match.</div>
              )}
            </div>
          ))}
          ...
          <button
            onClick={handleAddGroup}
            disabled={groups.length >= MAX_GROUPS}
            className={`flex items-center gap-1 text-xs w-full justify-center py-1.5 rounded-lg transition-all ${
              groups.length >= MAX_GROUPS
                ? 'bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-600 cursor-not-allowed'
                : 'bg-purple-100 dark:bg-purple-500/10 text-purple-700 dark:text-purple-300 hover:bg-purple-200 dark:hover:bg-purple-500/20'
            }`}
          >
          ...
          <button
            onClick={() => setEditing(false)}
            className="w-full bg-purple-500 text-white text-xs py-1.5 rounded-lg hover:bg-purple-600 transition-all active:scale-95 shadow-sm"
          >
            Done
          </button>
        </div>
        ...
        <div className="space-y-1">
          {groups.map((group) => (
            <div
              key={group.id}
              className="bg-white dark:bg-[#111827] text-xs p-1.5 rounded-lg border border-purple-100 dark:border-purple-500/20 flex items-center justify-between"
            >
              <span className="font-medium text-purple-700 dark:text-purple-300 truncate">
                {group.label || 'Unnamed'}
              </span>
              <span className="text-gray-500 dark:text-gray-400 ml-2 text-[10px] shrink-0">
                {group.keywords.length} {group.keywords.length === 1 ? 'keyword' : 'keywords'}
              </span>
            </div>
          ))}

          <div className="bg-gray-50 dark:bg-gray-800 text-xs p-1.5 rounded-lg border border-gray-200 dark:border-gray-700 flex items-center justify-between">
            <span className="text-gray-600 dark:text-gray-400 italic">Fallback (no match)</span>
            <span className="text-gray-400 dark:text-gray-500 ml-2 text-[10px] shrink-0">default</span>
          </div>
        </div>
      )}
