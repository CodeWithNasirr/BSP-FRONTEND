        ...
        try {
          ...
          // ✅ NEW: Handle broadcast completion
            ...
            payload.batch.forEach(update => {
              ...
              if (a === "refresh_chatlist") {
                if (!MODULE_IS_SELECTING) refresh(true);
              } else if (a === "new_message" && p) {
                moveToTop(p.recipient, p);
              } else if (a === "mark_read" && p) {
                markAsRead(p.recipient);
              } else if (a === "broadcast_result" && p) {
                setBroadcastResult({
                  sent: p.sent,
                  failed: p.failed,
                  total: p.total,
