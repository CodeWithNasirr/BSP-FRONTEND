...
//       // Append IST timezone offset (+05:30) to scheduled_time
//       const istDateTime = `${formData.scheduled_time}:00+05:30`; // Convert to ISO 8601 with IST offset
...
function CreateCampaigns() {
  ...
  const [isDisabled, setIsDisabled] = useState(true);
  const [templates, setTemplates] = useState([]);
  const [groups, setGroups] = useState([]); // Renamed for clarity
  const [contacts, setContacts] = useState([]);
  const [segments, setSegments] = useState([]); // New state for segments
  const [recipientType, setRecipientType] = useState('group');
  const [scheduleType, setScheduleType] = useState('immediate');
  const [formData, setFormData] = useState({
    template_name: '',
    ...
    const fetchData = async () => {
      try {
        const [templateRes, groupRes, contactRes, segmentRes] = await Promise.all([
          ...
          axios.get(`${API_BASE_URL}/api/add-group/`, { headers: { Authorization: `Token ${token}` } }),
          axios.get(`${API_BASE_URL}/api/contacts/`, { headers: { Authorization: `Token ${token}` } }),
          axios.get(`${API_BASE_URL}/api/segments/`, { headers: { Authorization: `Token ${token}` } }), // Fetch segments
        ]);
        setTemplates(templateRes.data.Data);
        setGroups(groupRes.data.data);
        setContacts(contactRes.data.data);
        setSegments(segmentRes.data.data); // Set segments
      } catch (err) {
        toast.error('Failed to fetch data');
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    // Add segment_id explicitly for segment recipient type
                ...
                <div className="grid gap-x-4 sm:gap-x-6 gap-y-4 sm:grid-cols-6 mb-8 capitalize">
                  ...
                  <div className="sm:col-span-3">
                    ...
                    >
                      <option value="group">Group/All Contacts</option>
                      <option value="single">Single Contact</option>
                      <option value="segment">Segment</option> {/* Added segment option */}
                    </select>
                  </div>
