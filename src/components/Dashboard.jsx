...
const Dashboard = () => { 
  const navigate = useNavigate();
  const location = useLocation(); // ✅ NEW: For detecting redirect params
  const { userInfo, isConnected, loadingUser, fetchDashboard } = useContext(Context);
  const token = localStorage.getItem("authToken");
  const user_email = userInfo.email || '';
  const user_name = userInfo.username || '';
  ...
  const [notificationsEnabled, setNotificationsEnabled] = useState(false);
  const [fbReady, setFbReady] = useState(false);
  const [isOnboarding, setIsOnboarding] = useState(false); // ✅ Loading state
  ...
  // ✅ Token exchange function (used by both popup callback and redirect handler)
  const exchangeTokenAndComplete = useCallback(async (code, sessionId) => {
    if (!code || !sessionId) {
      console.error("Missing code or sessionId");
      return;
    }
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        localStorage.removeItem("wa_onboarding_session");
        sessionStorage.removeItem("wa_onboarding_session");
      } else {
        toast.error("Connection failed: " + (response.data.error || "Unknown error"));
      }
    } catch (error) {
      console.error("Token exchange error:", error);
      toast.error("Token exchange failed: " + (error.response?.data?.error || error.message));
    } finally {
      setIsOnboarding(false);
    }
  }, [token, fetchDashboard]);

  // ✅ Handle OAuth redirect callback (for MOBILE)
  useEffect(() => {
    const handleRedirectCallback = async () => {
      ...
      if (code) {
        console.log("🔄 Detected OAuth redirect with code");
        
        // Clear URL params immediately to prevent re-processing
        window.history.replaceState({}, document.title, window.location.pathname);
        
        // Get session from storage (try both localStorage and sessionStorage)
        const storedSession = localStorage.getItem("wa_onboarding_session") || 
                             sessionStorage.getItem("wa_onboarding_session") ||
                             state; // Fallback to state from URL
        
        if (storedSession && token) {
          console.log("📱 Processing mobile redirect callback");
          await exchangeTokenAndComplete(code, storedSession);
        } else if (!token) {
          // User might need to login first, save the code temporarily
          console.log("⚠️ No auth token, saving code for after login");
          sessionStorage.setItem("pending_wa_code", code);
          sessionStorage.setItem("pending_wa_state", state || storedSession);
        } else {
          console.error("Missing session for redirect callback");
          toast.error("Session expired. Please try connecting again.");
        }
      }
      
      // Check for pending code after login
      const pendingCode = sessionStorage.getItem("pending_wa_code");
      const pendingState = sessionStorage.getItem("pending_wa_state");
      if (pendingCode && pendingState && token && !code) {
        console.log("📱 Processing pending WhatsApp connection");
        sessionStorage.removeItem("pending_wa_code");
        sessionStorage.removeItem("pending_wa_state");
        await exchangeTokenAndComplete(pendingCode, pendingState);
      }
    };
    
    handleRedirectCallback();
  }, [location.search, token, exchangeTokenAndComplete]);
  ...
  const startWhatsAppOnboarding = async () => {
    ...
    localStorage.setItem("wa_onboarding_session", sessionId);
    sessionStorage.setItem("wa_onboarding_session", sessionId);

    try {
      // Tell backend about this session
      await axios.post(
        `${API_BASE_URL}/api/start-whatsapp-onboarding/`,
        { session_id: sessionId },
        {
  ...
  useEffect(() => {
    ...
    // Message handler for embedded signup events (works on desktop)
    const messageHandler = (event) => {
      if (!event.origin.endsWith("facebook.com")) return;
      ...
      if (data.type === "WA_EMBEDDED_SIGNUP") {
        console.log("📱 WA_EMBEDDED_SIGNUP event received:", data.event);
        ...
        if (sessionId) {
          ...
          ).catch(err => console.error("Signup event error:", err));
        }
      }
    };
        ...
        <div className="md:max-h-screen flex flex-col w-full min-w-0 max-h-screen">
          <div className="bg-white md:bg-inherit p-4 md:p-8 rounded-[5px] text-[#000] h-full overflow-y-auto capitalize">
            ...
            <div className="flex flex-col sm:flex-row sm:space-x-2 space-y-2 sm:space-y-0 mb-8 text-xs md:text-sm">
              ...
              {/* ✅ WhatsApp Connect Button with loading state */}
              <button
                onClick={startWhatsAppOnboarding}
                disabled={isConnected || isOnboarding}
                className={`rounded-md cursor-pointer ${
                  isConnected 
                ...
                } px-3 py-2 text-sm font-semibold text-white shadow-sm text-center disabled:cursor-not-allowed`}
              >
                {isConnected 
                  ? 'Connected WhatsApp Successfully' 
                  : isOnboarding 
                  ? 'Connecting...' 
