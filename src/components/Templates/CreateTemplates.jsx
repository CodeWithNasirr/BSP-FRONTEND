...
const inputBaseClass = `
  block w-full rounded-md border-0 py-1.5 px-4 shadow-sm outline-none ring-1 ring-inset
...
function CreateTemplate() {
  ...
  const handleVariableFieldChange = (variable, field) => {
    setFormData((prev) => ({
      ...
      placeholder_mappings: {
        ...prev.placeholder_mappings,
        [variable]: { ...prev.placeholder_mappings[variable], field: field },
      },
    }));
  };
  ...
  const handleVariableExampleChange = (variable, example) => {
    setFormData((prev) => ({
      ...
      placeholder_mappings: {
        ...prev.placeholder_mappings,
        [variable]: { ...prev.placeholder_mappings[variable], example: example },
      },
    }));
  };
  ...
  const addButton = (type) => {
    ...
    const quickRepliesCount = formData.buttons.filter((b) => b.type === "QUICK_REPLY").length;
    const callbackCallCount = formData.buttons.filter(
      (b) => b.type === "CALLBACK" || b.type === "PHONE_CALL"
    ).length;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const formDataToSend = new FormData();
      Object.entries(formData).forEach(([key, value]) => {
        if (typeof value === "string" && value.trim() === "" && key !== "header_media_url") {
          return;
        }
        if (key === "buttons") return;
        if (key === "placeholder_mappings") {
  ...
  return (
    <RequireSubscription>
      <div className="main bg-zinc-50 dark:bg-[#111827] min-h-screen px-4 sm:px-6 transition-colors">
        <form onSubmit={handleSubmit}>
          <div className="header flex flex-col sm:flex-row justify-between px-4 sm:px-5">
            <div className="left py-5">
            ...
            <div className="right gap-2 px-4 sm:px-10 text-white flex flex-col sm:flex-row items-center">
              <button
                className="rounded-full bg-black dark:bg-gray-700 cursor-pointer px-5 py-2 text-white text-sm font-semibold flex items-center w-full sm:w-auto mb-2 sm:mb-0 transition-all active:scale-95"
                onMouseDown={() => navigate("/templates")}
                type="button"
              >
                Back
              ...
              <button
                className={`rounded-full px-5 py-2 text-white text-sm font-semibold flex items-center w-full sm:w-auto transition-all active:scale-95 shadow-sm ${
                  isDisabled
          ...
          <div className="container flex flex-col sm:flex-row w-full max-h-[500px]">
            {!isConnected && (
              <div className="w-full sm:md:w-[50%] p-4 sm:p-8 overflow-y-auto">
                <div className="bg-slate-50 dark:bg-[#111827] border border-primary shadow rounded-md p-4 py-8 border-gray-200 dark:border-gray-700 transition-colors">
                  <div className="flex justify-center mb-4 text-gray-900 dark:text-gray-100">
                  ...
                  <h4 className="text-center mb-4 text-gray-600 dark:text-gray-400">
                  ...
                  <div className="flex justify-center">
                    <Link
                      to={isConnected ? "#" : "/connect-form"}
                      onClick={(e) => isConnected && e.preventDefault()}
                      className={`rounded-md cursor-pointer px-3 py-2 text-sm font-semibold text-white shadow-sm mx-4 sm:mx-10 transition-all active:scale-95 ${
                        isConnected
            ...
            {isConnected && (
              <div className="w-full sm:md:w-[50%] p-4 sm:p-8 overflow-y-auto">
                <div className="grid gap-x-4 sm:gap-x-6 gap-y-4 sm:grid-cols-6 mb-8 capitalize">
                  <div className="sm:col-span-6">
                    <label htmlFor="template_name" className="block text-sm leading-6 text-gray-900 dark:text-gray-100">
                      Name
                    </label>
                    <div>
                      <input
                  ...
                  <div className="sm:col-span-3">
                    <label htmlFor="template_category" className="block text-sm leading-6 text-gray-900 dark:text-gray-100">
                      Category
                    </label>
                    <div className="relative">
                      <select className={selectBaseClass} value={formData.template_category} onChange={handleChange} name="template_category" id="template_category">
                  ...
                  <div className="sm:col-span-3">
                    <label htmlFor="template_language" className="block text-sm leading-6 text-gray-900 dark:text-gray-100">
                      Language
                    </label>
                    <div className="relative">
                      <select className={selectBaseClass} value={formData.template_language} onChange={handleChange} name="template_language" id="template_language">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-2 bg-[#f9f9fa] dark:bg-[#1a1a2e] rounded-lg mb-4 transition-colors">
                  {["TEXT", "IMAGE", "VIDEO", "DOCUMENT"].map((btn) => (
                    <button
                      type="button"
                      key={btn}
                      className={`text-center py-2 text-sm text-slate-800 dark:text-slate-200 m-1 transition-all ${
                        activeHeader === btn ? "bg-white dark:bg-[#111827] shadow rounded-lg" : ""
                      } cursor-pointer`}
                      onMouseDown={() => headerButtonClick(btn)}
                    >
                      {btn}
                  ...
                  {activeHeader && (
                    <button
                      type="button"
                      className="text-center py-2 text-sm text-slate-800 dark:text-slate-200 m-1 bg-red-100 dark:bg-red-500/20 hover:bg-red-200 dark:hover:bg-red-500/30 rounded-lg cursor-pointer transition-colors"
                      onMouseDown={clearHeader}
                    >
                      Clear
                    </button>
                ...
                {activeHeader === "TEXT" && (
                  <div className="sm:col-span-6">
                    <label htmlFor="header_text" className="block text-sm leading-6 text-gray-900 dark:text-gray-100">
                      Header Text
                    </label>
                    <input
                      value={formData.header_text}
                ...
                {["IMAGE", "VIDEO", "DOCUMENT"].includes(activeHeader) && (
                  <div className="sm:col-span-6">
                    <label className="block text-sm leading-6 text-gray-900 dark:text-gray-100">
                      Upload {activeHeader}
                    </label>
                    <input
                      name="header_media_url"
                      ...
                      onChange={handleChange}
                      className="block w-full rounded-md border-0 py-1.5 px-4 ring-1 ring-inset ring-gray-300 dark:ring-gray-700 bg-white dark:bg-[#111827] text-gray-900 dark:text-gray-100 text-sm transition-all"
                    />
                    {formData.header_media_url && (
                ...
                {variables.length > 0 && (
                  <div className="mt-4">
                    ...
                    {variables.map((variable, index) => (
                      <div key={index} className="mb-2 flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
                        <div className="flex-1 w-full">
                          <label className="text-slate-600 dark:text-slate-400 text-xs">[{variable}] - Field</label>
                          <input
                            list="contact-fields"
                            value={formData.placeholder_mappings[variable]?.field || ""}
                            onChange={(e) => handleVariableFieldChange(variable, e.target.value)}
                        ...
                        <div className="flex-1 w-full">
                          <label className="text-slate-600 dark:text-slate-400 text-xs">[{variable}] - Example</label>
                          <input
                            type="text"
                            placeholder="e.g., John"
                            value={formData.placeholder_mappings[variable]?.example || ""}
                ...
                <div className="flex gap-3 my-3">
                  <button
                    type="button"
                    onMouseDown={() => bottomButtonClick("PHONE_CALL")}
                    className="px-4 py-2 bg-slate-100 dark:bg-[#1a1a2e] text-gray-900 dark:text-gray-100 rounded-lg transition-colors"
                  >
                    Add Phone Call
                  </button>
                  <button
                ...
                {formData.buttons.length > 0 && (
                  <div className="mt-4">
                    <h4 className="text-gray-700 dark:text-gray-300 font-semibold mb-2">Added Buttons (Drag to reorder)</h4>
                    <DragDropContext
                      onDragEnd={(result) => {
                        if (!result.destination) return;
                        const updated = Array.from(formData.buttons);
                              ...
                              <Draggable key={idx} draggableId={`btn-${idx}`} index={idx}>
                                {(provided) => (
                                  <div
                                    ...
                                    {...provided.draggableProps}
                                    {...provided.dragHandleProps}
                                    className="flex items-center justify-between px-3 py-2 bg-blue-100 dark:bg-blue-500/20 rounded-lg cursor-move"
                                  >
                                    <span className="text-blue-700 dark:text-blue-300 text-sm">{btn.text} ({btn.type})</span>
                                    <button
                                      type="button"
                                      onClick={() => removeButton(idx)}
                                      className="text-red-500 font-bold hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 transition-colors"
                                    >
                                      ×
                                    </button>
                                  </div>
                ...
                {activeBottomButton === "PHONE_CALL" && isVisible && (
                  <div className="mt-4 mb-8">
                    <div className="bg-[#f9f9fa] dark:bg-[#1a1a2e] p-3 rounded-lg transition-colors">
                      <div className="flex items-center justify-between pb-4">
                        <span className="text-sm text-gray-900 dark:text-gray-100">Call Phone Number</span>
                        <button
                          type="button"
                          onMouseDown={() => setActiveBottomButton(null)}
                          className="bg-slate-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-full p-1 text-gray-700 dark:text-gray-300 transition-colors"
                        >
                          ✕
                        </button>
                      </div>
                      <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 border-t border-gray-200 dark:border-gray-700 pt-4">
                        <div className="flex-1">
                          <label className="block text-sm leading-6 text-gray-900 dark:text-gray-100 mb-1">Button Text</label>
                          <input type="text" name="button_text" value={formData.button_text} onChange={handleChange} placeholder="Call Now" className={inputBaseClass} />
                        </div>
                        <div className="flex-1">
                          <label className="block text-sm leading-6 text-gray-900 dark:text-gray-100 mb-1">Phone Number</label>
                          <input type="text" name="button_number" value={formData.button_number} onChange={handleChange} placeholder="Enter phone number" className={inputBaseClass} />
                        </div>
                      </div>
                      <button type="button" onMouseDown={() => addButton("PHONE_CALL")} className="mt-4 w-full bg-indigo-600 hover:bg-indigo-500 text-white py-2 rounded-lg transition-all active:scale-95 shadow-sm">
                        Add Button
                      </button>
                    </div>
                  </div>
                ...
                {activeBottomButton === "CALLBACK" && isVisible && (
                  <div className="mt-4 mb-8">
                    <div className="bg-[#f9f9fa] dark:bg-[#1a1a2e] p-4 rounded-lg transition-colors">
                      <div className="flex items-center justify-between pb-4">
                        <span className="text-sm text-gray-900 dark:text-gray-100">Website URL</span>
                        <button
                          type="button"
                          onMouseDown={() => setActiveBottomButton(null)}
                          className="bg-slate-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-full p-1 text-gray-700 dark:text-gray-300 transition-colors"
                      ...
                      <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 border-t border-gray-200 dark:border-gray-700 pt-4">
                        <div className="flex-1">
                          <label className="block text-sm leading-6 text-gray-900 dark:text-gray-100 mb-1">Button Text</label>
                          <input type="text" name="button_text" value={formData.button_text} onChange={handleChange} placeholder="Visit Website" className={inputBaseClass} />
                        </div>
                        <div className="flex-1">
                          <label className="block text-sm leading-6 text-gray-900 dark:text-gray-100 mb-1">Website URL</label>
                          <input type="text" name="button_url" value={formData.button_url} onChange={handleChange} placeholder="https://example.com" className={inputBaseClass} />
                        </div>
                      </div>
                      <button type="button" onMouseDown={() => addButton("CALLBACK")} className="mt-4 w-full bg-indigo-600 hover:bg-indigo-500 text-white py-2 rounded-lg transition-all active:scale-95 shadow-sm">
                        Add Button
                      </button>
                    </div>
                  </div>
                ...
                {activeBottomButton === "QUICK_REPLY" && isVisible && (
                  <div className="mt-4 mb-8">
                    <div className="bg-[#f9f9fa] dark:bg-[#1a1a2e] p-4 rounded-lg transition-colors">
                      <div className="flex items-center justify-between pb-4">
                        <span className="text-sm text-gray-900 dark:text-gray-100">Quick Reply Button</span>
                        <button
                          type="button"
                          onMouseDown={() => setActiveBottomButton(null)}
                          className="bg-slate-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-full p-1 text-gray-700 dark:text-gray-300 transition-colors"
                      ...
                      <label className="block text-sm leading-6 text-gray-900 dark:text-gray-100 mb-1">Button Text</label>
                      <input type="text" name="button_text" value={formData.button_text} onChange={handleChange} placeholder="Yes" className={inputBaseClass} />
                      <button type="button" onMouseDown={() => addButton("QUICK_REPLY")} className="mt-4 w-full bg-indigo-600 hover:bg-indigo-500 text-white py-2 rounded-lg transition-all active:scale-95 shadow-sm">
                        Add Button
                      </button>
                    </div>
                  </div>
            ...
            <div
              className="w-full sm:w-[30%] mx-auto sm:h-[80vh] p-4 sm:p-6 rounded-xl shadow-sm bg-cover bg-center border border-gray-200 dark:border-gray-700 transition-colors"
              style={{ backgroundImage: `url(${assest.whatsapp_bg})` }}
            >
              <div className="bg-white/90 dark:bg-[#111827]/90 rounded-xl p-3 h-full overflow-auto">
                <img
                  src="https://upload.wikimedia.org/wikipedia/commons/6/6b/WhatsApp.svg"
                  alt="WhatsApp Logo"
                  className="w-6 h-6 mb-4"
                  ...
                  <div className="mb-2">
                    {activeHeader === "IMAGE" && (
                      <img
                        src={URL.createObjectURL(formData.header_media_url)}
                        alt="Preview"
                        className="w-full max-w-[220px] rounded-lg border border-gray-200 dark:border-gray-700"
                      />
                    )}
                    {activeHeader === "VIDEO" && (
                      <video
                    ...
                    {activeHeader === "DOCUMENT" && (
                      <div className="flex items-center gap-2 p-2 bg-gray-100 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 w-[220px]">
                        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24">
                          <path fill="currentColor" d="M14 2H6c-1.1 0-2 .9-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8zm0 2l4 4h-4z" />
                        </svg>
                        <span className="text-xs truncate text-gray-700 dark:text-gray-300">{formData.header_media_url.name}</span>
                      </div>
                    )}
                  </div>
                )}
                ...
                {formData.header_text && (
                  <h1 className="font-semibold text-sm mb-1 text-gray-900 dark:text-gray-100">{formData.header_text}</h1>
                )}

                <span className="text-xs sm:text-sm whitespace-pre-wrap text-gray-800 dark:text-gray-200">
                  {formData.body_text}
                </span>

                {formData.footer_text && (
                  <div className="text-[11px] text-gray-500 dark:text-gray-400 mt-2">{formData.footer_text}</div>
                )}
                ...
                {formData.buttons.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {formData.buttons.map((btn, index) => (
                      <div key={index} className="flex justify-start items-center">
                        <span className="bg-white dark:bg-[#1f2937] rounded-full w-full py-2 text-center text-blue-500 dark:text-blue-400 text-xs sm:text-sm shadow-sm border border-gray-200 dark:border-gray-700 transition-colors">
                          {btn.text}
                        </span>
                      </div>
                    ))}
